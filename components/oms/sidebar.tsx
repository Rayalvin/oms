...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "sticky top-0 z-30 flex h-screen flex-col overflow-hidden border-r bg-white transition-[width] duration-300",
        isExpanded ? "w-[268px]" : "w-[78px]"
      )}
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2.5">
        {navModules.map((module) => {
          ...
          const modulePath = module.submodules?.[0]?.path ?? "/";
          const moduleActive = hasSubmodules
            ? module.submodules!.some((sub) => isPrefixActive(sub.path))
            : isPrefixActive(modulePath);
          const isOpen = openModules[module.id] ?? moduleActive;
          const headerClasses = cn(
            "w-full flex items-center rounded-2xl border transition-all duration-200",
            isExpanded ? "h-11 gap-3 px-3 justify-start" : "h-11 justify-center",
          );
          const headerStyle: React.CSSProperties = moduleActive
            ? { background: ACTIVE_BG, color: ACTIVE_FG, borderColor: "#DBEAFE" }
            : { background: MODULE_BASE_BG, color: "#64748B", borderColor: "#EEF2F7" };
                  ...
                  {module.submodules!.map((sub) => {
                    ...
                    return (
                      <Link
                        ...
                        href={sub.path}
                        className={cn(
                          "relative flex h-9 w-full items-center rounded-xl px-3 text-sm text-left transition-colors",
      ...
      <div className="border-t px-2 py-3 space-y-2" style={{ borderColor: "#EEF2F7" }}>
        <button
          className={cn(
            "w-full h-10 rounded-2xl flex items-center transition-colors hover:bg-[#F1F5F9]",
        ...
        <button
          className={cn(
            "w-full h-10 rounded-2xl flex items-center text-white shadow-sm transition-opacity hover:opacity-95",
