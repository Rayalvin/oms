...
export function TopBar({
  ...
  return (
    <header className="sticky top-0 z-20 bg-white px-8 py-5">
      {(breadcrumb || title) && (
        <div className="mb-3">
          {breadcrumb && (
      ...
      <div className="flex items-center gap-4 rounded-2xl bg-white">
        <div className="relative w-full max-w-[520px]">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="h-12 w-full rounded-2xl border-0 bg-[#F1F5F9] pl-11 pr-4 text-sm text-[#0F172A] outline-none placeholder:text-[#94A3B8] focus:ring-2 focus:ring-[#2563EB]/20"
          />
        </div>
        <div className="ml-auto flex items-center gap-1">
          {actions}
          <button className="h-10 w-10 rounded-xl text-[#64748B] transition-colors hover:bg-[#F1F5F9]">
